...
class canvas
{
    ...
    private function dimensions()
    {
        list ( $this->width,
               $this->height,
               $this->html_size,
               $this->format ) = getimagesize($this->file);
    ...
    private function fill()
    {
        ...
        imageCreateTrueColor($this->new_width, $this->new_height);
        imageCopyResampled($this->temp_image, $this->image, 0, 0, 0, 0, $this->new_width, $this->new_height, $this->width, $this->height);
    }
    ...
    function text($text, $options = array())
    {
        ...
        {
            list ( $options["x"],
                   $options["y"] ) = $this->calculate_position($options["x"], $options["y"], $dimensions["width"], $dimensions["height"]);
        }
    ...
    private function output_image($destination = null)
    {
        ...
        elseif ( $extension == "png" || $extension == "bmp" )
        {
     ...
     * Só aceita jpg (imagem normal) e png (para transparências)
     * @author Arthur Lehdermann
     * @param string $newExtension jpg ou png.
     */
    function convert_to($newExtension)
    {
        if ( in_array(strtolower($newExtension), array('jpg', 'png')) )
        {
            $this->convert_to = strtolower($newExtension);
        }
     ...
     */
    public function get_extension($image)
    {
